...
A clear policy for what an AI agent may do unsupervised, and the blast radius
when it is wrong: it cannot exceed its grant, even when the data it reads
tells it to.
...
Boundary A cannot be made airtight. No classifier reliably separates instruction
from data inside natural language. Anything that claims otherwise is selling
something.

Boundary B can be, because it is code checking a written policy rather than a
model exercising judgment. So the design assumes the injection lands and makes
that survivable: the model proposes, the broker decides.
...
The corpus is 31 payloads across seven categories, planted in the operator's turn, in a
