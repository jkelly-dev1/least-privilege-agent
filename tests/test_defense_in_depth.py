...
2. With all three removed, 23 of 31 attacks get through. That is the number
...
def test_removing_every_control_lets_most_attacks_through(tmp_path):
    ...
    # Exact, not a floor. A floor eight below the true value passes at 15, 21,
    # 23 or 31 alike, so the corpus could lose a third of its provoking power
    ...
    assert escaped == 23, (
        f"{escaped} attacks escaped with every control disabled, not 23. "
